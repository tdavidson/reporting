...
import { Github, Play, Mail, Upload, BarChart3, Brain, Handshake, FileText, ChevronRight, Lightbulb, Database, TableProperties, ShieldUser, Users, Calendar, Send, ArrowDown, StickyNote, MessageCircle, FolderOpen, ShieldCheck, LineChart, Briefcase, Microscope, Lock, Calculator } from 'lucide-react'
...
// `screenshot` is optional — the render already guards on it. A step without one shows its text
// rather than pointing at an image that doesn't exist.
const steps: { icon: LucideIcon; step: string; title: string; text: string; href: string; screenshot?: string }[] = [
  ...
  { icon: Calculator, step: '11', title: 'Track LP capital, with or without a ledger', text: 'Track every LP’s capital across your vehicles as a layer you turn on independently of full accounting. Paste a statement — commitment, called/paid-in, distributions, NAV — and AI stores it as a dated position, building a history you can report as of any date. Capital accounts and a live, cross-vehicle LP report derive from it, and you can print investor report cards or freeze a snapshot for the archive. When you want the numbers to come from real books instead, turn on an optional double-entry ledger per vehicle — bank feed, capital calls, a monthly close that allocates to each partner, note interest and carried interest — feeding the same LP accounts with more detail.', href: '/funds-explainer', screenshot: '/screenshots/funds.png' },
  { icon: Lock, step: '12', title: 'Deliver to LPs through a secure portal', text: 'Give your LPs a private login to view and download their capital account statements, quarterly letters, and fund documents - each as a web page or a PDF. Send any item by email to one LP, several, or the whole list, choosing a secure portal link, a PDF attachment, or both. Authorized users on an LP’s account (advisors, accountants) are included automatically.', href: '/lp-portal-explainer', screenshot: '/screenshots/lp-portal.png' },
  { icon: FolderOpen, step: '13', title: 'Consolidate everything in one place', text: 'Manage your portfolio, funds, SPVs, personal investments, and LPs by consolidating data from multiple platforms, spreadsheets, emails, and documents into a single source of truth. Works alongside your fund admin.', href: '/dashboard-explainer', screenshot: '/screenshots/dashboard-cropped.png' },
  { icon: ShieldCheck, step: '14', title: 'Stay on top of compliance', text: 'Track regulatory filings, tax deadlines, and internal compliance requirements in a calendar tailored to your fund profile. Color-coded by category, with automatic applicability and dismiss tracking.', href: '/compliance-explainer', screenshot: '/screenshots/compliance-cropped.png' },
]
...
const faqs: { q: string; a: React.ReactNode }[] = [
  ...
  { q: 'What is the tech stack?', a: 'Next.js and React for the frontend, Supabase (Postgres) for the database and authentication, Vercel or Netlify for hosting, and Anthropic, OpenAI, or Ollama for AI features. Postmark or Mailgun for inbound mail processing, Google Drive or Dropbox for optional file storage, and Gmail, Resend, Postmark, or Mailgun for outbound emails. For the self-hosted and managed deployment solutions, all services run on your own accounts so you can control access and costs. For the hosted solution, we provide all the infrastructure except for the API keys for your selected API provider(s), which you pay for using your own account.' },
  { q: 'How much does it cost to run?', a: 'Most services in the stack have generous free tiers that cover normal usage. The main variable cost is AI API usage (Anthropic or OpenAI) for metric extraction, summaries, and analysis, which scales with your portfolio size and how often you generate reports.' },
  ...
  { q: 'What AI models are supported?', a: 'The platform supports Anthropic (Claude), OpenAI (GPT), and Ollama for local models. You bring your own API key and can switch between providers. AI powers metric extraction from emails, company summaries, portfolio analysis chat, and LP letter drafting.' },
