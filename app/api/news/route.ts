...
async function classifyArticles(
  ...
  const articleList = articles.map((a, i) =>
    `[${i}] title: "${a.title}" | source: ${a.sourceDomain} | date: ${formatPubDate(a.pubDate)} | fetched_for_company: "${a.companyName}" | fetched_for_id: ${a.companyId}`
  ).join('\n')
...
Articles to classify (fetched_for_company = company whose name was used in the Google News search query):
${articleList}

For EACH article, return:
- "index": article index
- "companyId": use fetched_for_id as default. Only override if title clearly refers to a different portfolio company. Return null ONLY if article has zero connection to any portfolio company.
- "category": pick the MOST SPECIFIC category based on the article title:
    "rodada"      = funding round, captação, investimento recebido, Series A/B/C, seed, bridge
    "aquisicao"   = M&A, acquisition, compra de empresa, fusão, merger
    "parceria"    = partnership, parceria, integração, acordo comercial, joint venture
    "contratacao" = new hire, executive appointment, CEO/CFO/CTO change, layoffs, demissão, contratação
    "produto"     = product launch, new feature, lançamento de produto, nova funcionalidade, beta
    "expansao"    = geographic expansion, new market entry, expansão, internacionalização, nova cidade
    "premio"      = award, ranking, reconhecimento, certificação, prêmio, lista
    "crise"       = lawsuit, scandal, fraud, regulatory issue, falência, problema regulatório
    "ipo"         = IPO, abertura de capital, public offering, listagem em bolsa
    "outro"       = relevant to company but truly none of the above fit

Category examples for PT-BR titles:
- "Certta contrata Raphael Duarte como CFO" → "contratacao"
- "Asaas compra Mutuus e entra no mercado de seguros" → "aquisicao"
- "Asaas levanta R$100M em rodada Serie C" → "rodada"
- "BC autoriza Asaas a operar como financeira" → "outro"
- "Empresa muda nome para Certta" → "outro"
- "Asaas lança nova funcionalidade de pagamentos" → "produto"
- "Startup expande para Mexico" → "expansao"

Use "outro" ONLY as last resort. Always prefer a specific category.

Respond with ONLY a valid JSON array, no markdown, no extra text:
[{"index":0,"companyId":"uuid","category":"rodada"},{"index":1,"companyId":"uuid","category":"contratacao"}]`
...
async function reviewArticles(
  ...
  const prompt = `Review these news articles assigned to portfolio companies. Remove only clear false positives.

${articleList}

keep: true if company name (or variant) is in title, OR source domain matches company website.
keep: false if article is clearly about a completely different entity.
When in doubt → keep: true.

Respond ONLY with JSON array:
[{"index":0,"keep":true},{"index":1,"keep":false,"reason":"..."}]`
