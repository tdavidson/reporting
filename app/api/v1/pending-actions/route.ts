...
import { conversationLimit, ConversationRequestError } from '@/lib/api-v1/conversations'
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const result = await listPendingActions(admin, principal, {
      limit: conversationLimit(url.searchParams.get('limit')),
      cursor: url.searchParams.get('cursor'),
    })
    return v1Json(result, { requestId: id })
  } catch (error) {
    if (error instanceof V1PrincipalError || error instanceof PendingActionServiceError || error instanceof ConversationRequestError) {
      return v1Error(error.code, error.message, error.status, id)
    }
    console.error(`[api-v1] ${id} GET /pending-actions failed`, error)
