...
interface CommitmentEvent { lpEntityId: string; name: string; effectiveDate: string; amount: number; kind: string }
...
export function AllocationTermsView() {
  ...
  async function submitChange() {
    ...
    const ok = await post('/api/accounting/commitments', {
      lpEntityId: lp,
      effectiveDate,
      amount: amt,
    ...
    if (ok) { setLp(''); setFrom(''); setAmount(''); setMemo(''); setShowChange(false) }
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          ...
          <Button size="sm" variant="outline" onClick={() => setShowChange(v => !v)}>
            ...
            <div className="flex flex-wrap items-end gap-3">
              {isTransfer && (
                <label className="text-xs text-muted-foreground">From
                  <select value={from} onChange={e => setFrom(e.target.value)} className="mt-1 block h-9 px-2 rounded-md border border-input bg-background text-sm max-w-[200px]">
              ...
              <label className="text-xs text-muted-foreground">{isTransfer ? 'To' : 'Partner'}
                <select value={lp} onChange={e => setLp(e.target.value)} className="mt-1 block h-9 px-2 rounded-md border border-input bg-background text-sm max-w-[200px]">
                  <option value="">Select…</option>
                  {partners.map(p => <option key={p.lpEntityId} value={p.lpEntityId}>{p.name}</option>)}
                </select>
              </label>
              ...
              <Button size="sm" onClick={submitChange} disabled={busy}>
                {busy && <Loader2 className="h-3.5 w-3.5 mr-1 animate-spin" />}Record
              </Button>
            </div>
          </div>
        )}
