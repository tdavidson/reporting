...
import { Scale, ExternalLink, X, ChevronRight } from 'lucide-react'
...
const ISSUER_STYLES: Record<Issuer, {
  ...
  OTHER: { dot: 'bg-violet-500',  badge: 'bg-violet-500/10',  badgeText: 'text-violet-600 dark:text-violet-400', border: 'border-violet-500', label: 'Outro' },
}

const ORDER_CONFIG = [
  { key: 'firstOrder' as const,  label: '1ª Ordem', sub: 'Quem deve cumprir diretamente',     border: 'border-l-red-400' },
  { key: 'secondOrder' as const, label: '2ª Ordem', sub: 'Players afetados indiretamente',    border: 'border-l-amber-400' },
  { key: 'thirdOrder' as const,  label: '3ª Ordem', sub: 'Efeitos no ecossistema e startups', border: 'border-l-blue-400' },
]
...
function RegulationsTimeline({ regulations }: { regulations: Regulation[] }) {
  ...
  const years = useMemo(() => {
    const ys = Array.from(new Set(regulations.map(r => r.date.slice(0, 4)))).sort()
    return ys
  }, [regulations])
      ...
      <div className="border rounded-lg bg-card overflow-hidden">
        <div className="overflow-x-auto select-none" style={{ touchAction: 'pan-x' }}>
          <div className="relative flex items-center min-w-max px-6 py-6 gap-0">
            ...
            {filtered.length === 0 && (
              <p className="text-sm text-muted-foreground py-2 px-4">Nenhuma regulação para os filtros selecionados.</p>
            )}
            ...
            {filtered.map((reg, i) => {
              ...
              const dateLabel = new Date(reg.date + 'T00:00:00').toLocaleDateString('pt-BR', { month: 'short', year: 'numeric' })
        ...
        {selected && (
          <div className="border-t bg-muted/30 p-4 transition-all duration-200">
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {new Date(selected.date + 'T00:00:00').toLocaleDateString('pt-BR', { day: 'numeric', month: 'long', year: 'numeric' })}
            </p>
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  {new Date(reg.date + 'T00:00:00').toLocaleDateString('pt-BR', { month: 'short', year: 'numeric' })}
                </span>
              </div>
...
function ImpactFilterSection({ regulations }: { regulations: Regulation[] }) {
  ...
  // Agrupa por issuer para o Select
...
export function RegulacoesBRClient() {
  ...
  useEffect(() => {
    fetch('/api/regulacoes')
      .then(r => {
        if (!r.ok) throw new Error('Erro ao carregar regulações')
        return r.json()
      })
    ...
    <div className="p-4 md:py-8 md:px-8 space-y-12">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Linha do tempo de normas do BCB e CVM · 2014–2025
        </p>
      </div>
      ...
      {/* Seção 2 — Últimas 5 */}
      <section>
        <h2 className="text-lg font-semibold mb-4">5 Últimas Regulações</h2>
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      {/* Seção 3 — Análise de Impacto */}
      <section>
        <h2 className="text-lg font-semibold mb-1">Análise de Impacto por Regulação</h2>
        <p className="text-sm text-muted-foreground mb-4">
          Selecione uma norma para ver os desdobramentos em primeira, segunda e terceira ordem
          — quais mercados, indústrias e startups são impactados e por quê.
        </p>
        {loading ? (
