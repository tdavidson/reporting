...
- **`--primary` is the deployment's action colour** (fund-themeable). **`--brand` is OtherAdmin's** (highlighter yellow, the same as hemrock.com and nevermodel; marketing — a fill under ink text, never text itself; accent text is `text-brand-700 dark:text-brand-400`). They are not interchangeable.
