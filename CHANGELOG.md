...
Added
- **Text the Analyst.** Link a mobile number under Settings → Text the Analyst (a six-digit code is texted and typed back) and text questions to the fund's number; replies come back as texts. Same orchestrator, same live per-domain access, same conversation store as the panel and `/api/v1/chat`, with the model told it is writing a text and no write tools offered. `NEW` starts a fresh thread (one also starts after six quiet hours), `HELP` and `STOP`/`START` do what they say. Admins configure a Twilio number, Account SID and Auth Token under Settings → Text messaging; the webhook `/api/webhooks/sms/twilio` verifies Twilio's signature under that token and answers after acknowledging the delivery. Migration `20260904000006` adds the provider columns to `fund_settings` and the service-role-only `analyst_phone_numbers` and `analyst_phone_messages` tables
