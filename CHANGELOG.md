...
- **The Management section navigates like Funds.** Its subnav is entity-first once the URL names a management company — Overview, Bank, Journal, General ledger, Period close, Financial statements, all under `/manco/<id>/` — where before it had no children at all and the only way between a manco's books was back out to its lead page. Outside one there are no children: `/manco` is the entity list, and the firm-wide view of a section is `/funds/<section>`, which lists management companies too. `/manco/<section>` redirects there rather than 404ing
