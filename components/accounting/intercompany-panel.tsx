...
/**
 * Whose side the words are on. The data is always the MANAGEMENT COMPANY's (its receivables, its
 * charges, `direction` from its point of view); 'counterparty' renders the same rows as a fund
 * reads them — "we owe them", "they billed us" — so the fund's Admin page can show its charges
 * without a second implementation or a second set of postings.
 */
export type IntercompanyPerspective = 'manco' | 'counterparty'

export function IntercompanyPanel({
  vehicle, vehicleId, balances, charges, onChanged,
  perspective = 'manco', onlyCounterpartyId = null, mancoName,
}: {
  /** The management company's name (the `group` its routes take). */
  vehicle: string
  vehicleId: string
  balances: Balance[]
  charges: Charge[]
  onChanged: () => void
  perspective?: IntercompanyPerspective
  /** Show only the rows with this counterparty — the fund looking at its own charges. */
  onlyCounterpartyId?: string | null
  /** The management company's display name, for the counterparty view's phrasing. */
  mancoName?: string
}) {
  const currency = useCurrency()
  const fmt = (v: number) => formatCurrencyFull(v, currency)
  const fromFund = perspective === 'counterparty'
  const shownBalances = onlyCounterpartyId ? balances.filter(b => b.counterpartyVehicleId === onlyCounterpartyId) : balances
  const shownCharges = onlyCounterpartyId ? charges.filter(c => c.counterpartyVehicleId === onlyCounterpartyId) : charges
  const firm = mancoName ?? vehicle
    ...
    <Card>
      <CardContent className="pt-4 pb-4 px-4">
        <div className="flex items-center justify-between gap-2 mb-3">
          <p className="text-sm font-medium">{fromFund ? `Intercompany with ${firm}` : 'Intercompany'}</p>
        ...
        {shownBalances.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            {fromFund
              ? <>No balance with {firm}. Recording a charge posts both sides &mdash; the expense and the payable here, the receivable and the income on {firm}&rsquo;s books.</>
              : <>No intercompany balances. Recording a charge posts both sides &mdash; the receivable and the income here, the expense and the payable on the other vehicle&rsquo;s books.</>}
          </p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b text-left text-caption uppercase text-muted-foreground">
                  <th className="py-1.5 pr-3 font-normal">{fromFund ? 'With' : 'Counterparty'}</th>
                  {/* Due from and due to are shown SEPARATELY and only then netted. A counterparty
                      confirms a payable and a receivable, not a net — see intercompany.ts. The
                      manco's "owes us" is the fund's "we owe"; the columns swap with the view. */}
                  <th className="py-1.5 px-3 text-right font-normal">{fromFund ? 'They owe us' : 'Owes us'}</th>
                  <th className="py-1.5 px-3 text-right font-normal">{fromFund ? 'We owe them' : 'We owe'}</th>
              ...
              <tbody>
                {shownBalances.map(b => (
                  <tr key={b.counterpartyVehicleId || b.counterpartyName} className="border-b last:border-0">
                    <td className="py-2 pr-3">{fromFund ? firm : b.counterpartyName}</td>
                    <td className="py-2 px-3 text-right tabular-nums">{fmt(fromFund ? b.dueTo : b.dueFrom)}</td>
                    <td className="py-2 px-3 text-right tabular-nums">{fmt(fromFund ? b.dueFrom : b.dueTo)}</td>
                    <td className="py-2 pl-3 text-right tabular-nums font-medium">{fmt(fromFund ? -b.net : b.net)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {shownCharges.length > 0 && (
          <div className="mt-4">
            ...
            <div className="divide-y">
              {shownCharges.slice(0, 12).map(c => (
                <div key={c.id} className="flex items-center gap-3 py-2">
                  {(c.direction === 'receivable') !== fromFund
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm">
                      ...
                      <span className="text-muted-foreground">
                        {' '}&middot; {fromFund
                          ? (c.direction === 'receivable' ? `billed by ${firm}` : `billed to ${firm}`)
                          : `${c.direction === 'receivable' ? 'billed to' : 'billed by'} ${nameFor(c.counterpartyVehicleId)}`}
                      </span>
                    </p>
        ...
        <ChargeDialog
          ...
          fixedCounterpartyId={onlyCounterpartyId}
          firm={firm}
...
function ChargeDialog({
  open, onOpenChange, vehicle, vehicleId, onPosted, perspective = 'manco', fixedCounterpartyId = null, firm,
}: {
  open: boolean
  ...
  onPosted: () => void
  perspective?: IntercompanyPerspective
  /** From a fund's page the counterparty is the fund itself — fixed, not chosen. */
  fixedCounterpartyId?: string | null
  firm?: string
}) {
  const fromFund = perspective === 'counterparty'
  const [counterparties, setCounterparties] = useState<Counterparty[]>([])
  const [counterpartyId, setCounterpartyId] = useState(fixedCounterpartyId ?? '')
  ...
  useEffect(() => {
    ...
    fetch(`/api/manco/intercompany?group=${encodeURIComponent(vehicle)}`)
      ...
      .then(d => {
        ...
        setCounterpartyId(prev => fixedCounterpartyId ?? prev ?? (list[0]?.id ?? ''))
      })
      .catch(() => setCounterparties([]))
  }, [open, vehicle, vehicleId, fixedCounterpartyId])
        ...
        <div className="space-y-3">
          <div className="space-y-1">
            ...
            <div className="inline-flex rounded-md border p-0.5 text-xs">
              {/* `direction` is always the management company's. From a fund's page the same
                  two values read the other way round — which is exactly why the form uses words. */}
              {([
                ['receivable', fromFund ? `${firm} billed us` : 'We billed them'],
                ['payable', fromFund ? `We billed ${firm}` : 'They billed us'],
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              disabled={!!fixedCounterpartyId}
              className="w-full rounded-md border border-input bg-background px-2 py-1.5 text-sm disabled:opacity-70"
            >
